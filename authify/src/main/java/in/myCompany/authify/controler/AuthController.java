...
import in.myCompany.authify.io.AuthRequest;
import in.myCompany.authify.io.ResetPasswordRequest;
import in.myCompany.authify.service.AppUserDetailsService;
import in.myCompany.authify.service.ProfileService;
import in.myCompany.authify.util.JwtUtil;
...
public class AuthController {
     ...
     public ResponseEntity<?> login(@RequestBody AuthRequest request) {
          try {
               ...
               return ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, cookie.toString())
                       .body(new AuthRequest(request.getEmail(),jwtToken));

          } catch (BadCredentialsException e) {

               Map<String, Object> error = new HashMap<>();
               error.put("error", true);
               error.put("message", "Email or password is incurred");
     ...
     public void sendResetOtp(@RequestParam String email){
          try{
               profileService.sendResentOtp(email);
          }catch (Exception e){
